...
************************************************************* */
-- SELECT top 10000 * from BD_BACKUP_OGEI.dbo.TRAMA_BASE_NIÑOS_GESTANTES
...
select id_cita
	...
	, edad_reg edad 
into #his_minsa
-- from BD_BACKUP_OGEI.dbo.TRAMA_BASE_NIÑOS_GESTANTES with (nolock)
from BD_BACKUP_OGEI.dbo.TramaHisMinsa with (nolock)
where sw=1																	-- Validación del numero de documento.
...
(
	...
	(	convert(int,cod_estab) in (select distinct convert(int,[CODIGO_UNICO]) 
							from [dbo].[FED_2024_25_EESS_POBLACION_ASIGNADA_20240808_ANTIGUO])  )	-- EESS de Segundo Nivel con Pob Asignada.
) 
...
where id_cita in (select distinct id_cita from #his_minsa where cod_item in ('90749.01','90749.02'	 -- Covid
																			...
																			,'90658', '90688'		 -- Vacunación contra la Influenza (Adulto (Estacional))
																			,'90717'				 -- Vacunación Antiamarílica (AMA) 
																			,'90670'				 -- Vacunación Antineumocócica
																			,'90749.05'				 -- Vacunacion Dengue
																			)				 
				)
and year(fecha_atencion)=@año
and month(fecha_atencion)=@mes_inicio
...
where converT(int,renaes) in (	select distinct converT(int,renaes)
								from dbo.FED_2024_25_EESS_EXCLUIDOS_CSMC_ESPECIALIZADOS_20240730_ANTIGUO)		-- Centros Comunitarios de Salud Mental y otros
...
where TipoCondicion_Personsa_Estab in ('N','R')						-- Adolescentes atendidas (Nuevas o Reingresantes).	
and  year(fecha_atencion)=@año
...
if object_id(N'FED25.TRAMAHIS_FED2024_25_SI0401_Nominal_2025', N'U') is not null drop table FED25.TRAMAHIS_FED2024_25_SI0401_Nominal_2025;
--if Object_id(N'tempdb..#TRAMAHIS_FED2024_25_SI0401_Nominal',N'U') is not null drop table #TRAMAHIS_FED2024_25_SI0401_Nominal;
...
, a.*
-- into #TRAMAHIS_FED2024_25_SI0401_Nominal
into FED25.TRAMAHIS_FED2024_25_SI0401_Nominal_2025
...
-- from #TRAMAHIS_FED2024_25_SI0401_Nominal
from FED25.TRAMAHIS_FED2024_25_SI0401_Nominal_2025
